...
# ================================================================================
columns = st.selectbox("Choose a column to fill", df.select_dtypes(include = "number").columns)

method = st.radio("Choose a method", ["Original DF",
                                      "Impute Mean",
                                      "Impute Median",
                                      "Impute Zero"])
...
with col1:
    ...
    plt.title(f"Original Distribution of {columns}")
    st.pyplot(fig)
...
with col2:
    ...
    plt.title(f"Distribution of {columns} after {method}")
    st.pyplot(fig)
    st.subheader(f"{columns}'s New Stats")
